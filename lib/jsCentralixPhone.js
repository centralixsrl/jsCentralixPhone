...
class centralixVoicePhone extends EventManager {
    ...
    handleNewRTCSession(ev) {
        ...
        const callerName = (ev.session.direction === 'incoming') ? newSession.remote_identity.display_name : '';
        ...
        newSession.on('progress', (e) => {
            this.trigger('onRinging', {sessionID : ev.session.id, direction : ev.session.direction, originator : e.originator, callerid : callerID});
        });

        newSession.on('confirmed', (e) => {
            this.trigger('onConfirmed', {sessionID : ev.session.id, direction : ev.session.direction, originator : e.originator, callerid : callerID, ack : e.ack});
        });
